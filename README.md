...
```
初始权重 = 时间基础权重 + 随机扰动
         = avg_value × (0.85 ~ 1.15)  // 基于文件修改时间
         + avg_value × random(-0.15, +0.15)  // 随机扰动
```

**总体范围**：`avg_value ± 30%`（如基础 100 时，新壁纸权重在 70~130 之间）

**设计目的**：
- **时间戳映射**：较新的文件获得较高的基础权重（用户通常更想看新添加的壁纸）
- **随机扰动**：即使文件是同时复制的（时间戳相同），也能产生差异化权重
- **避免同权值问题**：确保容差选择时有足够的区分度
