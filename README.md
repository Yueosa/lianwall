# 🎬 LianWall

智能动态壁纸管理器 - 基于负反馈闭环调节的壁纸轮换系统

###### 省流

> **这是一个有记忆的智能壁纸轮换器** 🎯
> 
> - 刚播放过的壁纸会"进入冷却"，短期内不会重复
> - 长期未播放的壁纸会"积攒期望"，逐渐获得出场机会
> - 支持 **动态壁纸**（视频）和 **静态壁纸**（图片）两种模式
> - 权重数据持久化，重启后继续接着之前的状态轮换
> - 适配 Hyprland，使用 mpvpaper 和 swww 作为底层引擎
> 
> **最佳使用场景：** 5～30 个壁纸时算法最稳定（更多壁纸支持开发中）

## ✨ 特性

- **负反馈闭环调节** - 有记忆的随机，避免重复和埋没
- **空间化选择逻辑** - 二分切割算法，自然有机的播放序列
- **多引擎支持** - mpvpaper（动态）/ swww（静态）
- **持久化权重** - 重启后保留壁纸"地形"状态

## 📦 安装

> 推荐放在 `~/.local/bin` 下

###### 依赖

LianWall 依赖以下壁纸引擎，请确保已安装：

- **[mpvpaper](https://github.com/GhostNaN/mpvpaper)** - 动态壁纸引擎，基于 mpv 播放视频壁纸
- **[swww](https://github.com/LGFae/swww)** - 静态壁纸引擎，支持丰富的过渡动画

```bash
# Arch Linux
paru -S mpvpaper swww

# 或手动编译安装
```

###### 自行编译

```bash
cargo build --release
cp target/release/lianwall ~/.local/bin/
sudo chmod +x ~/.local/bin/lianwall
```

###### 下载 Release 包

```bash
cp ./lianwall_{实际版本号}_linux_x86_64 ~/.local/bin/lianwall
sudo chmod +x ~/.local/bin/lianwall
```

## 🚀 使用

```bash
lianwall daemon              # 启动守护进程（动态壁纸模式，循环切换）
lianwall next                # 立即切换到下一张壁纸（根据当前模式）
lianwall video               # 切换到动态壁纸模式（自动停止 swww）
lianwall picture             # 切换到静态壁纸模式（自动停止 mpvpaper）
lianwall kill                # 停止所有壁纸引擎（mpvpaper + swww）
lianwall reset -m <mode>     # 热重载指定模式的壁纸目录
lianwall status -m <mode>    # 显示指定模式的状态和壁纸列表
```

### Hyprland 配置

```conf
# 随 Hyprland 启动
exec-once = lianwall daemon

# 快捷键
bind = SUPER ALT, W, exec, lianwall next # 下一张壁纸
bind = SUPER ALT, E, exec, lianwall video # 视频模式
bind = SUPER ALT, Q, exec, lianwall kill # 停止壁纸
bind = SUPER ALT, R, exec, lianwall picture # 图片模式
```

## ⚙️ 配置

配置文件位置：`~/.config/lianwall/config.toml`

首次运行会自动生成默认配置，可按需修改：

```toml
[paths]
video_cache = "~/.cache/lianwall/video.json"    # 动态壁纸权重缓存
image_cache = "~/.cache/lianwall/image.json"    # 静态壁纸权重缓存
video_dir = "~/Videos/background"               # 动态壁纸目录
image_dir = "~/Pictures/wallpapers"             # 静态壁纸目录

[video_engine]
type = "mpvpaper"      # 动态壁纸引擎
interval = 600         # 切换间隔（秒），默认 10 分钟

[image_engine]
type = "swww"                # 静态壁纸引擎
interval = 300               # 切换间隔（秒），默认 5 分钟
transition = "fade"          # 过渡效果: fade, left, right, top, bottom, wipe, wave, grow, center, any, outer, random
transition_duration = 2.0    # 过渡时长（秒）

[weight]
base = 100.0           # 基础权重
select_penalty = 10.0  # 选中惩罚值
skip_reward_max = 5.0  # 最大跳过奖励
```

**配置说明：**
- 两种模式（动态/静态）使用独立的缓存文件和目录
- `video_engine` 控制动态壁纸行为（mpvpaper）
- `image_engine` 控制静态壁纸行为（swww），支持丰富的过渡效果
- `weight` 参数影响权重算法，建议保持默认值

---

## 🧠 算法设计解析

### 一、核心算法：负反馈闭环与确定性中位选择

LianWall 的核心是一个**权重驱动的确定性选择系统**，通过负反馈机制实现有记忆的伪随机轮换。

#### 1.1 权重更新规则

```rust
每次切换后，对所有壁纸进行权重调整：

被选中壁纸：
  value_new = value_old - 10.0          // 惩罚固定为 -10
  skip_streak = 0                        // 重置跳过计数

未被选中壁纸：
  skip_streak += 1                       // 累计跳过次数
  reward = f(skip_streak)                // 阶梯函数
  value_new = value_old + reward         // 累加奖励
```

**跳过奖励函数 f(n)：**

$$
f(n) = \begin{cases}
1.0 + \epsilon & \text{if } 1 \leq n \leq 2 \\
2.0 + \epsilon & \text{if } 3 \leq n \leq 4 \\
3.0 + \epsilon & \text{if } 5 \leq n \leq 6 \\
4.0 + \epsilon & \text{if } 7 \leq n \leq 8 \\
5.0 + \epsilon & \text{if } n \geq 9
\end{cases}
$$

其中 $\epsilon \sim U(-0.3, 0.3)$ 为微小随机扰动，防止权重完全相等导致的排序不稳定。

#### 1.2 选择算法：容差中位策略

传统贪心算法总是选择最高权重项，会导致初始权重略高的文件被过度选择。LianWall 采用**容差中位选择**：

```rust
Algorithm: ToleranceMedianSelect(wallpapers, tolerance=5.0)
  Input:  wallpapers[] 壁纸列表
          tolerance    容差值（默认 5.0）
  Output: 选中的索引

  1. 按权重降序排序：wallpapers.sort_by(value, DESC)
  2. max_value ← wallpapers[0].value
  3. candidates ← {i | max_value - wallpapers[i].value ≤ tolerance}
  4. return candidates[len(candidates) / 2]  // 整数除法
```

**关键特性：**
- **容差过滤**：筛选出权重在 `[max - 5, max]` 范围内的所有候选项
- **中位选择**：从候选集中取中间位置（整数除法），而非最高值
- **确定性**：相同权重分布 → 相同选择结果（无随机因素）
- **伪随机性**：通过权重微扰 ε 引入不可预测性

**示例演示：**
```
排序后权重: [105.2, 103.1, 102.3, 101.8, 100.1, 95.4, 90.0]
max = 105.2
容差范围: [100.2, 105.2]
候选索引: [0, 1, 2, 3, 4]  (5个候选项)
选择: candidates[5/2] = candidates[2] → 权重 102.3 的壁纸
```

---

### 二、数学性质与系统稳定性分析

#### 2.1 权重守恒与膨胀率

设系统有 $N$ 个壁纸，第 $t$ 轮总权重为 $W_t$：

$$
W_{t+1} = W_t - 10 + \sum_{i \neq \text{selected}} f(n_i)
$$

假设各壁纸跳过次数分布均匀，平均奖励 $\bar{f} \approx 2.5$（考虑阶梯函数分布）：

$$
W_{t+1} \approx W_t - 10 + (N-1) \times 2.5 = W_t + 2.5N - 12.5
$$

**膨胀率分析：**
- $N = 5$：$\Delta W = 0$（零增长）
- $N = 10$：$\Delta W = +12.5$（轻微膨胀）
- $N = 20$：$\Delta W = +37.5$（膨胀）
- $N = 50$：$\Delta W = +112.5$（强膨胀）

**结论：** 系统设计为"轻微膨胀"，避免权重归零导致的选择退化，适合 5-30 个壁纸规模。

#### 2.2 冷却期与重复间隔估算

壁纸被选中后，权重从 $v$ 降至 $v - 10$。要恢复竞争力，需满足：

$$
v - 10 + \sum_{k=1}^{t} f(k) \geq v_{\text{threshold}}
$$

**案例计算（初始权重 100）：**

```
选中后: 100 → 90
第1轮:  90 + 1.0 = 91.0
第2轮:  91 + 1.0 = 92.0
第3轮:  92 + 2.0 = 94.0
第4轮:  94 + 2.0 = 96.0
第5轮:  96 + 3.0 = 99.0  ← 接近初始值
第6轮:  99 + 3.0 = 102.0 ← 超过初始值
```

**恢复到竞争力时间：**
- 恢复到初始值：约 **5-6 轮**
- 超过未选中壁纸：约 **7-8 轮**（考虑其他壁纸也在累积奖励）

若切换间隔 10 分钟，冷却期约 **50-80 分钟**。

#### 2.3 埋没问题的数学证明

**定理：** 在 LianWall 系统中，任何壁纸的跳过次数 $n$ 不会无限增长。

**证明：**

假设壁纸 $A$ 连续 $n$ 轮未被选中，其权重为：

$$
v_A(n) = v_0 + \sum_{k=1}^{n} f(k)
$$

由于 $f(k)$ 单调递增且在 $k \geq 9$ 时封顶为 5.0，有：

$$
v_A(n) \geq v_0 + \min\left(\frac{n(n+1)}{2}, 5n - 36\right)
$$

当 $n$ 足够大时，$v_A(n)$ 将严格大于其他壁纸，必然进入容差范围并被选中。

**推论：** 系统保证**公平性**——不存在永久埋没的壁纸（无限等待问题不存在）。

---

### 三、容差中位选择的优势

#### 3.1 与贪心算法对比

| 算法类型     | 选择策略       | 优势                           | 劣势                         |
| ------------ | -------------- | ------------------------------ | ---------------------------- |
| **贪心选择** | 总是选最高权重 | 最优权重利用                   | 易受初始条件影响，缺乏多样性 |
| **容差中位** | 容差范围的中位 | 打破初始偏差，平衡多样性与质量 | 可能跳过绝对最优项           |
| **随机选择** | 均匀随机       | 完全无偏                       | 无记忆，可能短期重复         |

#### 3.2 中位选择的"反贪心"效应

考虑初始权重场景：
```
文件A.mp4: 120.0（修改时间最新）
文件B.mp4: 119.8
文件C.mp4: 119.5
文件D.mp4: 100.0
文件E.mp4: 80.0
```

**贪心算法：** 总是选 A → A 被惩罚后，总是选 B → B 被惩罚后，总是选 C → ...

**容差中位：** 筛选 [115, 120] 范围 → 候选 [A, B, C] → 选择 **B（中位）**

结果：A 和 C 获得"缓冲机会"，避免初始排序带来的确定性重复序列。

---

### 四、初始权重分配策略

新文件权重基于修改时间线性映射：

$$
v_{\text{init}}(t) = 120 - 40 \times \frac{t - t_{\text{newest}}}{t_{\text{oldest}} - t_{\text{newest}}}
$$

其中 $t$ 为文件修改时间。结果：
- **最新文件**：120.0（+20% 竞争优势）
- **平均文件**：100.0（基准）
- **最旧文件**：80.0（-20% 竞争劣势）

**合并策略（热重载时）：**
```rust
新发现文件权重 = (时间权重 + 现有平均权重) / 2
```

避免新文件突然涌入打破现有生态平衡。

---

### 五、系统参数调优建议

| 参数              | 默认值 | 适用场景       | 调整建议                      |
| ----------------- | ------ | -------------- | ----------------------------- |
| `select_penalty`  | 10.0   | 通用           | 增大 → 冷却期更长             |
| `skip_reward_max` | 5.0    | 通用           | 减小 → 恢复更慢，轮换更均匀   |
| `tolerance`       | 5.0    | 固定（代码内） | 增大 → 候选池更大，随机性增强 |
| `base`            | 100.0  | 基准值         | 不建议修改                    |

**实战示例（20个壁纸，10分钟间隔）：**
- 理论轮换周期：约 **3-4 小时**
- 单个壁纸冷却期：约 **1 小时**
- 长期均衡状态：权重收敛至 $[90, 130]$ 区间

---

### 六、与传统方案对比

| 特性           | `shuf` 随机  | 加权随机     | LianWall 确定性中位 |
| -------------- | ------------ | ------------ | ------------------- |
| **记忆性**     | ❌ 无         | ✅ 概率分布   | ✅ 权重状态持久化    |
| **冷却保证**   | ❌ 可能连续   | ⚠️ 概率性     | ✅ 确定性惩罚        |
| **公平性**     | ⚠️ 长期收敛   | ✅ 理论公平   | ✅ 数学证明公平      |
| **可预测性**   | 完全不可预测 | 部分可预测   | 确定性+微扰         |
| **初始偏差**   | ❌ 文件名依赖 | ⚠️ 初始权重   | ✅ 中位选择打破偏差  |
| **持久化成本** | -            | 需存概率分布 | JSON 权重文件       |

---

### 七、算法复杂度

- **时间复杂度**：$O(N \log N)$（排序主导）
- **空间复杂度**：$O(N)$（权重数组）
- **I/O 复杂度**：每次切换 1 次写入（JSON 序列化）

**优化空间：**
- 使用堆维护 Top-K 候选项：$O(N + K \log K)$，适合 $N > 100$ 场景
- 增量更新排序：利用权重变化局部性减少排序开销

---

## 📁 项目结构

```
src/
├── main.rs             # 总入口，命令分发
├── config.rs           # 配置文件解析
├── manager.rs          # WallManager 核心逻辑
├── command.rs          # 命令行解析 (clap)
├── paperengine/        # 壁纸引擎
│   ├── mod.rs          # PaperEngine trait
│   ├── mpvpaper.rs     # 动态壁纸 (视频)
│   └── swww.rs         # 静态壁纸 (图片)
└── algorithm/          # 算法模块
    ├── mod.rs
    ├── weight.rs       # 权重计算
    └── selector.rs     # 二分选择
```

## 📜 License

MIT
